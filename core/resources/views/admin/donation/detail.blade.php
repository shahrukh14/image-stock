            ...
            <div class="card b-radius--10 overflow-hidden box--shadow1">
                <div class="card-body">
                    ...
                    <ul class="list-group">
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            @lang('Amount')
                            <span class="fw-bold">{{ showAmount($donation->amount) }} {{ __($general->cur_text) }}</span>
                        </li>
                       
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            @lang('After Charge')
                            <span class="fw-bold">{{ showAmount($donation->amount + $donation->charge) }} {{ __($general->cur_text) }}</span>
                        </li>
                    ...
                    <div class="mt-4">
                        <button class="btn btn-outline--success btn-sm ms-1 confirmationBtn" data-action="{{ route('admin.deposit.approve', $donation->deposit->id) }}" data-question="@lang('Are you sure to approve this transaction?')"><i class="las la-check-double"></i>
                            @lang('Approve')
                        </button>
                        <button class="btn btn-outline--danger btn-sm ms-1 rejectBtn" data-id="{{$donation->deposit->id}}"  data-amount="{{ showAmount($donation->amount) }} {{ __($general->cur_text) }}" data-username="{{ @$donation->user->username }}"><i class="las la-ban"></i> @lang('Reject')
                        </button>
                    @endif
                    </div>
                </div>
