...
@section('panel')
    <div class="row justify-content-center">
        <div class="col-md-12">
            <form action="{{ route('admin.vectors.update', $vector->id) }}" method="post">
                @csrf
                <div class="card">
                    <div class="card-body">
                        <div class="row">
                                ...
                                <div class="row">
                                    @if (@$vector->user)
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <input class="form-control" type="text" value="{{ __($vector->user->fullname) }}" disabled>
                                            </div>
                                        </div>
                                    @endif
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control select2-tokenize" name="category[]"  multiple="multiple" required>
                                                @foreach ($categories as $category)
                                                    <option value="{{ $category->id }}" @selected($vector->category_id && in_array($category->id, $vector->category_id))>{{ __($category->name) }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            ...
                                            <input class="form-control" name="title" type="text" value="{{ $vector->title }}" required>
                                        </div>
                                    </div>
                                            ...
                                            <div class="form-group" id="extension">
                                                ...
                                                <select class="form-control select2-tokenize" name="colors[]" multiple="multiple" required>
                                                    @foreach ($colors as $color)
                                                        <option value="{{ $color->color_code }}" @selected($vector->colors && in_array($color->color_code, $vector->colors))>{{ __($color->name) }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                            ...
                                            <div class="form-group" id="extension">
                                                ...
                                                <select class="form-control select2-tokenize" name="extensions[]" multiple="multiple" required>
                                                    @foreach ($extensions as $option)
                                                        <option value="{{ $option }}" @selected($vector->extensions && in_array($option, $vector->extensions))>{{ __(strtoupper($option)) }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input class="form-control" type="text" value="{{ $vector->total_view }}" disabled>
                                </div>
                            </div>
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input class="form-control" type="text" value="{{ $vector->total_like }}" disabled>
                                </div>
                            </div>
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input class="form-control" type="number" value="{{ $vector->totalDownloads }}" disabled>
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input name="attribution" data-width="100%" data-size="large" data-onstyle="-success" data-offstyle="-danger" data-bs-toggle="toggle" data-height="35" data-on="@lang('Enable')" data-off="@lang('Disable')" type="checkbox" @if ($vector->attribution) checked @endif>
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control status" name="status" required>
                                        ...
                                        @if ($vector->status == 0)
                                            <option value="0" @selected($vector->status == 0)>@lang('Pending')</option>
                                        @endif
                                        <option value="3" @selected($vector->status == 3)>@lang('Rejected')</option>
                                        <option value="1" @selected($vector->status == 1)>@lang('Approved')</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-12">
                                                    ...
                                                    <div class="col-md-2">
                                                        <div class="form-group">
                                                            ...
                                                            <div class="input-group">
                                                                ...
                                                                <a class="input-group-text" href="{{ route('admin.vectors.file.download', $value->id) }}">
                            ...
                            <div class="col-12">
                                <ul class="list-group my-3">
                                    <li class="list-group-item bg--primary border--primary text-center">
                                        <span class="fw-bold text--white">@lang('Thumb Vectors')</span>
                                    </li>
                                    <li class="list-group-item d-flex justify-content-between flex-wrap">
                                        <div class="col-12 extraPriceElement">
                                            <div class="row align-items-center">
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control nicEdit" name="description" rows="5" required>{{ $vector->description }}</textarea>
                                </div>
                            </div>
                            ...
                            <div class="row reason">
                                <div class="border-bottom my-3 text-center">
                                    ...
                                    @if ($vector->admin_id || $vector->reviewer_id)
                                        <h6 class="mb-2">@lang('Previously Reviewed By') {{ $vector->admin_id ? $vector->admin->name : $vector->reviewer->name }}</h6>
                                    @endif
                                </div>
                                <div class="col-md-6">
                                    <div class="form- group">
                                ...
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control" name="reason" rows="6" @if ($vector->status == 3) required @endif>{{ $vector->reason }}</textarea>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-12">
...
@push('breadcrumb-plugins')
    <x-back route="{{ route('admin.vectors.all') }}" />
@endpush
