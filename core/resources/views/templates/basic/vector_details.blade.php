                    ...
                    <div style="transform: translate3d(0px, 0%, 0px) scale3d(1, 1, 1) rotateX(0deg) rotateY(0deg) rotateZ(0deg) skew(0deg, 0deg); opacity: 1; transform-style: preserve-3d;" class="flex-horizontal start breadcrumbs">
                            ...
                            <div class="w-dyn-list">
                                <div role="list" class="w-dyn-items">
                                    <div role="listitem" class="w-dyn-item">
                                        <a href="{{ route('vectors') }}" class="text-link text-200 medium" style="text-decoration: none;">Vectors</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            <div class="w-dyn-list">
                                <div role="list" class="w-dyn-items">
                                    <div role="listitem" class="w-dyn-item">
                                        <a href="{{ route('vectors', 'category='.$vector->Category->name ) }}" class="text-link text-200 medium" style="text-decoration: none;">{{$vector->Category->name}}</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="text-200 medium">{{$vector->title}}</div>
                    </div>
                </div>
                <div class="w-layout-grid grid-2-columns product-page">
                        ...
                        <div style="opacity: 1;" class="position-relative">
                            ...
                            @else
                                <div class="image-wrapper product-image">
                                    <img alt="{{$vector->title}}"   src="{{ imageUrl(getFilePath('stockImage'), $vector->image_name) }}" class="image background-image">
                                </div>
                            @endif
                        </div>
                    </div>
                    <div style="transform: translate3d(0px, 0%, 0px) scale3d(1, 1, 1) rotateX(0deg) rotateY(0deg) rotateZ(0deg) skew(0deg, 0deg); opacity: 1; transform-style: preserve-3d;" class="product-page-main-content">
                        <div class="product-page-main-content---top">
                            <h1 class="display-3 mg-bottom-4px">{{ __($vector->title) }}</h1>
                            <p class="mg-bottom-0" style="word-wrap: break-word;">@php echo $vector->description; @endphp</p>
                        </div>
                        ...
                        <div class="product-details-wrapper">
                            <div>
                                <div class="flex-horizontal start gap-12px flex-wrap">
                                    <div>
                                        <div class="heading-h6-size mg-bottom-2px">{{($vector->user->fullname) }}</div>
                                        <div class="text-100 medium">{{date('M d, Y', strtotime($vector->upload_date))}}</div>
                                    </div>
                                </div>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <div class="flex-horizontal start gap-6px">
                                    <img src="{{ asset('assets\images\app_images\_type-icon-stock-x-webflow-template.svg') }}" alt="Size">
                                     <div class="text-100 medium color-neutral-700">{{ (implode(' | ', $vector->categoryName($vector->category_id))) }}</div>
                                </div>
                            </div>
                            <div>
                                <div class="mg-bottom-8px">
                                ...
                                <div class="flex-horizontal start gap-6px">
                                    ...
                                    @if ($vector->extensions)
                                        <div class="text-100 medium color-neutral-700">{{ __(strtoupper(implode(' | ', $vector->extensions))) }}</div>
                                    @endif
                                </div>
                            </div>
            ...
            <div class="container-default w-container">
                <div style=" transform: translate3d(0px, 0%, 0px) scale3d(1, 1, 1)  rotateX(0deg) rotateY(0deg) rotateZ(0deg) skew(0deg, 0deg); opacity: 1; transform-style: preserve-3d;" class="mg-bottom-32px keep">
                    <div class="w-layout-grid grid-2-columns title-and-buttons">
                        <h2 class="display-3 mg-bottom-0">Related Vectors&Graphics</h2>
                        <div class="buttons-row">
                            <a href="{{ route('vectors') }}" class="browseAllVectors"> </a>
                        </div>
                    </div>
                </div>
                <div class="w-dyn-list">
                    <div role="list" class="grid-3-columns _1-col-tablet gap-row-48px w-dyn-items">
                        @foreach ($relatedVectors as $relatedVector)
                        @php
                            $vectorUrl = imageUrl(getFilePath('stockImage'), $relatedVector->thumb);
                            $defaultImage = getImage('assets/images/frontend/default_images/' . @$defaultImageContent->data_values->loading_image);
                        @endphp
                       <div role="listitem" class="w-dyn-item">
                        <a  href="{{ route('image.detail', [slug($relatedVector->title), $relatedVector->id]) }}" class="resource-card-wrapper w-inline-block">
                          <div class="image-wrapper">
                            <img alt="Image" class="gallery__img lazy-loading-img" data-image_src="{{ $vectorUrl }}" src="{{ $defaultImage }}"/>
                          </div>
                          <div class="resource-card-content v2">
                            <div class="text-200 color-neutral-100 mg-bottom-24px">
                              #{{$relatedVector->track_id}}
                            </div>
                            <div class="mg-top-auto">
                              <div class="flex-horizontal space-between gap-16px">
                                <div  class="flex-horizontal start gap-12px flex-wrap">
                                  <div class="avatar-circle _02">
                                    <img src="{{ getImage(getFilePath('userProfile') . '/' . $relatedVector->user->image, null, 'user') }}" alt="{{$relatedVector->user->firstname}}" />
                                  </div>
                                  <div>
                                    <div class="heading-h6-size color-neutral-100">
                                      {{$relatedVector->user->firstname}}
                                    </div>
                                    <div class="text-50 color-neutral-300">
                                      {{date('d-M-Y', strtotime($relatedVector->upload_date))}}
                                    </div>
                                  </div>
                                </div>
...
<style>
    ...
    .browseAllVectors {
        background: url("/core/public/assets/image/buttons/browse resources black.png") no-repeat;
        background-size: 100% 100%;
        padding: 23px 110px;
        color: transparent;
    }

    .browseAllVectors:hover {
        background: url("/core/public/assets/image/buttons/browse resources green.png") no-repeat;
        background-size: 100% 100%;
        padding: 23px 110px;
        color: transparent;
    }
