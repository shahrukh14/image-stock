...
class ProcessController extends Controller
{
    ...
    public static function process($deposit)
    {
        ...
        $val['name'] = auth()->user()->username ?? $deposit->donation->sender->name;
        $val['description'] = "Payment By Razorpay";
        $val['image'] = getImage(getFilePath('logoIcon') .'/logo.png');
        $val['prefill.name'] = auth()->user()->firstname . ' ' . auth()->user()->lastname ?? $deposit->donation->sender->name;
        $val['prefill.email'] = auth()->user()->email ?? $deposit->donation->sender->email;
        $val['prefill.contact'] = auth()->user()->mobile ?? $deposit->donation->sender->mobile;
    ...
    public function ipn(Request $request)
    {
        ...
        $deposit = Deposit::where('btc_wallet', $request->razorpay_order_id)->with('donation.image')->orderBy('id', 'DESC')->first();
