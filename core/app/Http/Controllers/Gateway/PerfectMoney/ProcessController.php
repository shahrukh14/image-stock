...
class ProcessController extends Controller
{
    ...
    public static function process($deposit)
    {
        ...
        $val['PAYMENT_URL'] = route(...$successRedirectUrl);
        $val['PAYMENT_URL_METHOD'] = 'POST';
        $val['NOPAYMENT_URL'] = route(...$failedRedirectUrl);
        $val['NOPAYMENT_URL_METHOD'] = 'POST';
        $val['SUGGESTED_MEMO'] = auth()->user()->username ?? $deposit->donation->sender->name;
    ...
    public function ipn()
    {
        $deposit = Deposit::where('trx', $_POST['PAYMENT_ID'])->with('donation.image')->orderBy('id', 'DESC')->first();
