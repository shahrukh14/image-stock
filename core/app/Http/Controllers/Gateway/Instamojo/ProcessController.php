...
class ProcessController extends Controller
{
    ...
    public static function process($deposit)
    {
        ...
        $payload = array(
            ...
            'buyer_name' => $deposit->user->username ?? $deposit->donation->sender->name,
            'redirect_url' => route(...$failedRedirectUrl),
            'webhook' => route('ipn.'.$deposit->gateway->alias),
            'email' => $deposit->user->email ?? $deposit->donation->sender->email,
            'send_email' => true,
    ...
    {
        $deposit = Deposit::where('btc_wallet', $_POST['payment_request_id'])->where('status', Status::PAYMENT_INITIATE)->orderBy('id', 'DESC')->with('donation.image')->first();
        $instaMojoAcc = json_decode($deposit->gatewayCurrency()->gateway_parameter);
