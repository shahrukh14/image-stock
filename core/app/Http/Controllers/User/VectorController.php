...
class VectorController extends Controller
{
    ...
    public function all(){
        ...
        return view($this->activeTemplate . 'user.vector.list', compact('pageTitle', 'vectors'));
    }
    ...
    protected function vectorData($scope = null){
        $user   = auth()->user();
        $vectors = Image::where('user_id', $user->id)->where('file_type', 'vector');

        if ($scope) {
            $vectors = $vectors->$scope();
        }

        return $vectors->withSum('files as total_downloads', 'total_downloads')->orderBy('id', 'desc')->paginate(getPaginate(21));
    }

    public function add(){
        $pageTitle  = "Upload Vector";
        ...
        return view($this->activeTemplate . 'user.vector.upload', compact('pageTitle', 'categories', 'tags'));
    }
    ...
    public function store(Request $request){
        ...
        $vector   = new Image();
        $response = $this->processVectorData($vector, $request);
    ...
    public function edit($id){
        $vector      = Image::where('user_id', auth()->id())->with('files')->findOrFail($id);
        $pageTitle  = 'Update vector - ' . $vector->title;
        $categories = Category::active()->orderBy('name')->get();
        $colors     = Color::all();

        $vectors = Image::approved()
            ->inrandomOrder()
            ->pluck('tags')
            ->toArray();
        $tags = array_slice(array_unique(array_merge(...$vectors)), 0, 50);
        return view($this->activeTemplate . 'user.vector.upload', compact('pageTitle', 'categories', 'colors', 'vector','tags'));
    }
    ...
    public function updateVector(Request $request, $id){
        ...
        $vector = Image::where('user_id', $user->id)->find($id);
        ...
        if ($general->storage_type == 1) {
            if ($request->hasFile('photo')) {
                $photo      = getFilePath('stockImage') . '/' . $vector->image_name;
                $photoThumb = getFilePath('stockImage') . '/' . $vector->thumb;
                removeFile($photo);
                removeFile($photoThumb);
            }
        }

        $response = $this->processVectorData($vector, $request, true);
    ...
    public function changeActiveStatus($id){
        $vector = Image::where('user_id', auth()->id())->findOrFail($id);
        $vector->is_active = $vector->is_active ? Status::DISABLE : Status::ENABLE;
        $vector->save();

        $notification = 'Vector deactivated successfully';
        if ($vector->is_active) {
            $notification = 'Vector activated successfully';
        }
        $notify[] = ['success', $notification];
        return back()->withNotify($notify);
    }

    protected function processVectorData($image, $request, $isUpdate = false){
        ...
        $notification = 'Vector uploaded successfully';
        if ($isUpdate) {
            $notification = 'Vector updated successfully';
        }
