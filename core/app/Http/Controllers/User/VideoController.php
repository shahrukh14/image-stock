...
class VideoController extends Controller
{
    ...
    public function rejected(){
        $pageTitle = "Rejected Video";
        $videos    = $this->videoData('rejected');
        return view($this->activeTemplate . 'user.video.list', compact('pageTitle', 'videos'));
    }

    public function approved(){
        $pageTitle = "Approved Video";
    ...
    protected function videoData($scope = null){
        $user   = auth()->user();
        $videos = Image::where('user_id', $user->id)->where('file_type', 'video');

        if ($scope) {
            $videos = $videos->$scope();
        }
        return $videos->withSum('files as total_downloads', 'total_downloads')->orderBy('id', 'desc')->paginate(getPaginate(21));
    }
    ...
    public function add(){
        ...
        return view($this->activeTemplate . 'user.video.upload', compact('pageTitle', 'categories', 'tags'));
    }
    ...
    public function store(Request $request){
        ...
        $video   = new Image();
        $response = $this->processVideoData($video, $request);
    ...
    public function edit($id){
        $video      = Image::where('user_id', auth()->id())->with('files')->findOrFail($id);
        $pageTitle  = 'Update video - ' . $video->title;
        $categories = Category::active()->orderBy('name')->get();
        $colors     = Color::all();

        $videos = Image::approved()
            ->inrandomOrder()
            ->pluck('tags')
            ->toArray();
        $tags = array_slice(array_unique(array_merge(...$videos)), 0, 50);
        return view($this->activeTemplate . 'user.video.upload', compact('pageTitle', 'categories', 'colors', 'video','tags'));
    }
    ...
    public function updateVideo(Request $request, $id){
        ...
        $video = Image::where('user_id', $user->id)->find($id);
        ...
        if ($general->storage_type == 1) {
            if ($request->hasFile('photo')) {
                $photo      = getFilePath('stockImage') . '/' . $video->image_name;
                $photoThumb = getFilePath('stockImage') . '/' . $video->thumb;
                removeFile($photo);
                removeFile($photoThumb);
            }
        }

        $response = $this->processVideoData($video, $request, true);
    ...
    public function changeActiveStatus($id){
        $video = Image::where('user_id', auth()->id())->findOrFail($id);
        $video->is_active = $video->is_active ? Status::DISABLE : Status::ENABLE;
        $video->save();

        $notification = 'Video deactivated successfully';
        if ($video->is_active) {
            $notification = 'Video activated successfully';
        }
        $notify[] = ['success', $notification];
        return back()->withNotify($notify);
    }

    protected function processVideoData($image, $request, $isUpdate = false){
